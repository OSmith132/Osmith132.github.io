...
async function fetchRepoData(repoOwner, repoName) {
  ...
  fetch(`https://api.github.com/users/${repoOwner}`)
    ...
    .catch(error => {
      ...
      document.getElementById('profile-pic').src = 'assets/icons/warning-icon.png';
    });
  ...
  async function fetchTotalCommits() {
    ...
    while (true) {
      const response = await fetch(`https://api.github.com/repos/${repoOwner}/${repoName}/commits?per_page=${perPage}&page=${page}`);
